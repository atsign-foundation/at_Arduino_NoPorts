#include "atauth/run_enroll_command.h"
#include "atauth/apkam_keys.h"
...
// Portable file-exists check (works on ESP32 + POSIX)
...
int atauth_enroll_command(const char *atsign, const char *root_domain, const char *keys_path, const char *passcode,
  ...
  if (keys_path == NULL) {
    ...
    if (atkeys_path != NULL && file_exists(atkeys_path)) {
    ...
    if (keys_path != NULL && file_exists(keys_path)) {
      atlogger_log(TAG, ATLOGGER_LOGGING_LEVEL_ERROR, "atKeys file already exists at: %s\n", keys_path);
      return 1;
    }
  ...
  ret = atclient_utils_find_atserver_address(root_domain, 64, atsign, &auth_opts.atserver_host,
                                             &auth_opts.atserver_port);
  if (ret != 0) {
  ...
  ret = fetch_and_decrypt_key(&atclient.atserver_connection, ATAUTH_ENCRYPTED_SELF_ENC_KEY_NAME,
                              response.enrollment_id, atsign, apkam_keys.apkam_symmetric_key.symmetric_key_raw,
                              &self_encryption_key);
